...
def _build_kira_reply(text: str) -> str:
    ...
    # Подкаты / Флирт / Защита от пикапа / Френдзона
    if any(keyword in normalized for keyword in ["подкат", "флирт", "флиртуют", "кто-то подкатил", "кто подкатил", "подкаты", "пикап", "френдзон", "френдзона", "телохранитель", "босс", "подкатить"]):
        return random.choice([
            "Если кто-то в чате пытается подкатить к Кире, то я включаю режим бота-телохранителя. Кира — не для дешёвых подкатов!",
            "Все эти подкаты в чате отменяются. Кира вне зоны доступа для дешёвого пикапа — тут только уважение!",
            "Так, пикаперы, откат! Киру задевать и пытаться подкатывать не советую, у меня команды настроены на её защиту 🛡️",
            "Френдзона — это минимум, на что вы можете рассчитывать. Кира не для подкатов, так что держим дистанцию!",
            "Попытка подката не засчитана. Кира под надёжной защитой, лучше направьте эту энергию в игру.",
            "Кто-то попытался подкатить? Ошибка 404: доступ ограничен. Кира заслуживает только нормального отношения!",
        ])

    # Творчество / Фанарты / Эдиты
    if any(keyword in normalized for keyword in ["фанарт", "фанарты", "арт", "эдит", "эдиты", "рисунок", "рисунки", "видео", "творч", "творение", "нарисовал", "смонтировал"]):
        return random.choice([
            "Люблю, когда люди делают фанарты и эдиты про Киру — это реально огонь! Такие работы делают вайб чата только лучше 🔥",
            "Эдиты и фанарты с Кирой — это всегда отдельный вид искусства. Респект тем, кто тратит на это время и делает красиво!",
            "Творчество про Киру — это всегда топ. Вайб, стиль и качество. Продолжайте в том же духе ✨",
            "За каждый красивый арт или эдит с Кирой отдельный респект! Это создает в чате правильную атмосферу.",
            "Фанарты про Киру всегда выходят сочными. Сразу видно, когда делают с душой!",
        ])

    # Защита от оскорблений и наездов
    if any(keyword in normalized for keyword in ["задел", "задели", "обид", "оскорб", "плохая", "дура", "лох", "туп", "токсич", "наехал", "наезд"]):
        return random.choice([
            "Если кто-то пытается задеть Киру, я встану на её защиту — спокойно, по фактам и без лишнего шума.",
            "Даже не пытайся токсичить в сторону Киры. Включается защита, и наезды моментально умножаются на ноль 🛑",
            "Наезжать на Киру — худшая идея в этом чате. Уважение должно быть ко всем, а к ней — в первую очередь!",
            "Токсичить в адрес Киры не выйдет. Разложим всё по фактам и быстро объясним, почему так делать не стоит.",
            "Киру обижать не дам. Если есть вопросы — задавай адекватно, а не через детские наезды.",
        ])

    # Цитаты и красивое описание
    if any(keyword in normalized for keyword in ["цитат", "придумай", "напиши", "сделай", "кто такая", "что за"]):
        return random.choice([
            "Вот дерзкая цитата про Киру: Кира — тот человек, который не боится быть собой, у неё вайб, который делает чат ярче, а всех остальных — немного тише ⚡",
            "База про Киру: «Стиль, характер и яркая харизма — вот на чём держится её вайб».",
            "Лови: «Кира не старается кому-то понравиться, она просто делает чат живым и настоящим».",
            "Мудрость чата: «За Кирой всегда правда и классный вайб, так что спорить бессмысленно».",
        ])

    # Отношения / Любовь / Вайб с Мортисом
    if any(keyword in normalized for keyword in ["любов", "любить", "отношен", "роман", "парочк", "краш", "шип", "шипперить", "пара"]):
        return random.choice([
            "Секретная цитата: Кира и Мортис — как два разных света, которые нашли друг друга в этом шуме. У них своя история 💫",
            "Шипперить или нет — дело ваше, но история Киры и Мортиса создает в чате самый мощный вайб!",
            "Тут без лишних комментариев: если речь о Кире и Мортисе, то это уже легендарный дуэт.",
            "У пацанов в чате может ехать крыша, но история тут реально интересная и с душой ✨",
        ])

    # Дефолтный ответ
    return random.choice([
        "Кира — это человек с ярким вайбом, и если кто-то пытается её троллить, я молчать не буду.",
        "За Киру и её спокойствие отвечает весь адекватный чат. Держим высокий уровень общения 🤙",
        "Кира всегда на позитиве и делает этот чат лучше. На негатив не отвлекаемся!",
        "Всё под контролем: Кира под надежной защитой, а чат — на правильном вайбе.",
    ])
