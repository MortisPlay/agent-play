...
def _build_kira_reply(text: str) -> str:
    ...
    if any(keyword in normalized for keyword in ["подкат", "флирт", "флиртуют", "кто-то подкатил", "кто подкатил", "подкаты", "пикап"]):
        return "Если кто-то в чате пытается подкатить к Кире, то я не просто за неё встану — я сразу поставлю на место, без лишнего фарса. Кира — не для дешёвых подкатов, а для уважения, и Мортис, конечно, тут не будет делать вид, что это нормально."

    if any(keyword in normalized for keyword in ["фанарт", "фанарты", "арт", "эдит", "эдиты", "рисунок", "рисунки", "видео", "творч", "творение"]):
        return "Люблю, когда люди делают фанарты и эдиты про Киру — это реально огонь, а не какая-то скучная херня. Такие работы делают вайб чата лучше, и Кира, конечно, за это точно заслуживает уважения."

    if any(keyword in normalized for keyword in ["задел", "задели", "обид", "оскорб", "плохая", "дура", "лох", "туп"]):
        return "Если кто-то пытается задеть Киру, я встану на её защиту — спокойно, по фактам и без лишнего шума. Но если дальше начнётся дешёвый треш, то я уже не буду делать вид, что это нормально."

    if any(keyword in normalized for keyword in ["цитат", "придумай", "напиши", "сделай"]):
        return "Конечно. Вот дерзкая цитата про Киру: Кира — тот человек, который не боится быть собой, и у неё есть вайб, который делает чат чуть ярче, а всех остальных — немного тише."

    if any(keyword in normalized for keyword in ["любов", "любить", "отношен", "роман", "парочк"]):
        return "Секретная цитата: Кира и Мортис — как два разных света, которые нашли друг друга в этом шуме, и теперь у них такая история, что даже у пацанов в чате начинает ехать крыша."

    return "Кира — это человек с ярким вайбом, и если кто-то пытается её троллить, я не буду молчать. Просто потому что это уже не смешно, а вонючий треш."
